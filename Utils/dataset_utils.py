...
def load_batch_from_celebA_dir(indices: list, normalize: bool = True):
    """Loads a batch from CelebA
    ...
    batched_images = np.zeros((batch_size, 218, 178, 3), dtype=np.uint8)
    for i in range(batch_size):
        batched_images[i] = plt.imread(os.path.join(
            CELEBA_DIR, f"{str(indices[i]+1).zfill(6)}.jpg"))
    if normalize:
        batched_images = batched_images.astype(np.float32) / 255.
